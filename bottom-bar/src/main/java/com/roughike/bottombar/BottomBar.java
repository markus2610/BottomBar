...
public class BottomBar extends FrameLayout
   ...
   */
  @Deprecated public static BottomBar attachShy(CoordinatorLayout coordinatorLayout,
   ...
   * @param fragmentItems an array of {@link BottomBarFragment} objects.
   */
  @Deprecated public void setFragmentItems(android.support.v4.app.FragmentManager fragmentManager,
      @IdRes int containerResource, BottomBarFragment... fragmentItems) {
    if (fragmentItems.length > 0) {
      int index = 0;
   ...
   */
  @Deprecated public void setOnItemSelectedListener(OnTabSelectedListener listener) {
    mListener = listener;
  }
    ...
    if (mBadgeMap != null && mBadgeMap.size() > 0) {
      ...
      for (Integer key : mBadgeMap.keySet()) {
        BottomBarBadge badgeCandidate =
            (BottomBarBadge) mOuterContainer.findViewWithTag(mBadgeMap.get(key));
   ...
   */
  @Deprecated public void useDarkTheme(boolean darkThemeEnabled) {
  ...
  public BottomBarBadge makeBadgeForTabAt(int tabPosition, int backgroundColor, int initialCount) {
    ...
    if (mIsComingFromRestoredState && mBadgeStateMap != null && mBadgeStateMap.containsKey(
        tabPosition)) {
  ...
  /**
   * Don't resize the tabs when selecting a new one, so every tab is the same0 if you have more than
   * three
   * tabs. The text still displays the scale animation and the icon moves up, but the badass width
   * animation
   * is ignored.
   */
            ...
            @SuppressWarnings("deprecation") @Override public void onGlobalLayout() {
              if (!mShyHeightAlreadyCalculated) {
                ((CoordinatorLayout.LayoutParams) getLayoutParams()).setBehavior(
                    new BottomNavigationBehavior(getOuterContainer().getHeight(), 0, isShy(),
                        mIsTabletMode));
              }
  ...
  private boolean handleLongClick(View v) {
    if ((mIsShiftingMode || mIsTabletMode) && v.getTag().equals(TAG_BOTTOM_BAR_VIEW_INACTIVE)) {
      Toast.makeText(mContext, mItems[findItemPosition(v)].getTitle(mContext), Toast.LENGTH_SHORT)
          .show();
    }
  ...
  private void onRestoreInstanceState(Bundle savedInstanceState) {
    if (savedInstanceState != null) {
      ...
      mBadgeStateMap =
          (HashMap<Integer, Boolean>) savedInstanceState.getSerializable(STATE_BADGE_STATES_BUNDLE);
  ...
  private void unselectTab(View tab, boolean animate) {
    ...
    if (animate) {
      ViewPropertyAnimatorCompat titleAnimator =
          ViewCompat.animate(title).setDuration(ANIMATION_DURATION).scaleX(scale).scaleY(scale);
  ...
  private static void navBarMagic(Activity activity, final BottomBar bottomBar) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH && ViewConfiguration.get(
            ...
            @SuppressWarnings("deprecation") @Override public void onGlobalLayout() {
              ...
              if (bottomBar.isShy()) {
                ...
                ((CoordinatorLayout.LayoutParams) bottomBar.getLayoutParams()).setBehavior(
                    new BottomNavigationBehavior(newHeight, defaultOffset, bottomBar.isShy(),
