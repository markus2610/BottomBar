...
public class BottomBar extends FrameLayout
    ...
    private SparseArray<Object> mBadgeMap;
    private SparseBooleanArray mBadgeStateMap;
     ...
     * <p/>
     * Remember to also call {@link #onRestoreInstanceState(Bundle)} inside of your {@link
     * Activity#onSaveInstanceState(Bundle)} to restore the state.
     *
     ...
     */
    public static BottomBar attach(@NonNull Activity
            activity, Bundle savedInstanceState) {
    ...
    /**
     * Bind the BottomBar to the specified View's parent, and inflate your layout there. Useful when
     * the BottomBar overlaps some content that shouldn't be overlapped.
     * <p/>
     * Remember to also call {@link #onRestoreInstanceState(Bundle)} inside of your {@link
     * Activity#onSaveInstanceState(Bundle)} to restore the state.
     *
    ...
    /**
     * Deprecated. Breaks support for tablets. Use {@link #attachShy(CoordinatorLayout, View,
     * Bundle)} instead.
     */
    @Deprecated
    public static BottomBar attachShy(CoordinatorLayout coordinatorLayout,
            Bundle savedInstanceState) {
        return attachShy(coordinatorLayout, null, savedInstanceState);
    }

    /**
     * Adds the BottomBar inside of your CoordinatorLayout and shows / hides it according to scroll
     * state changes.
     * <p/>
     * Remember to also call {@link #onRestoreInstanceState(Bundle)} inside of your {@link
     * Activity#onSaveInstanceState(Bundle)} to restore the state.
    ...
    public static BottomBar attachShy(CoordinatorLayout coordinatorLayout, View userContentView,
        ...
        if (userContentView != null && coordinatorLayout.getContext()
                .getResources()
                .getBoolean(R.bool.bb_bottom_bar_is_tablet_mode)) {
    ...
    /**
     * Set tabs and fragments for this BottomBar. When setting more than 3 items, only the icons
     * will show by default, but the selected item will have the text visible.
     *
     ...
     */
    public void setFragmentItems(android.app.FragmentManager fragmentManager,
            @IdRes int containerResource, BottomBarFragment... fragmentItems) {
            ...
            for (BottomBarFragment fragmentItem : fragmentItems) {
                if (fragmentItem.getFragment() == null
                    ...
                    throw new IllegalArgumentException(
                            "Conflict: cannot use android.app.FragmentManager " +
     ...
     * Deprecated.
     * <p/>
     * Use either {@link #setItems(BottomBarTab...)} or {@link #setItemsFromMenu(int,
     * OnMenuTabClickListener)} and add a listener using {@link #setOnTabClickListener(OnTabClickListener)}
     * to handle tab changes by yourself.
     * <p/>
     * Set tabs and fragments for this BottomBar. When setting more than 3 items, only the icons
     * will show by default, but the selected item will have the text visible.
     *
     ...
     */
    @Deprecated
    public void setFragmentItems(android.support.v4.app.FragmentManager fragmentManager,
            @IdRes int containerResource, BottomBarFragment... fragmentItems) {
        if (fragmentItems.length > 0) {
            int index = 0;
            ...
            for (BottomBarFragment fragmentItem : fragmentItems) {
                if (fragmentItem.getSupportFragment() == null
                    ...
                    throw new IllegalArgumentException(
                            "Conflict: cannot use android.support.v4.app.FragmentManager " +
    ...
    /**
     * Set tabs for this BottomBar. When setting more than 3 items, only the icons will show by
    ...
    public void setItemsFromMenu(@MenuRes int menuRes,
            OnMenuTabSelectedListener listener) {
        clearItems();
    ...
    public void setDefaultTabPosition(int defaultTabPosition) {
        ...
        if (mItems == null) {
            ...
            return;
        } else if (mItems.length == 0
                || defaultTabPosition > mItems.length - 1
    ...
    /**
     * Set the maximum number of tabs, after which the tabs should be shifting ones with a
     * background color.
     * <p/>
    ...
    /**
     * Always show the titles and icons also on inactive tabs, even if there's more than three of
     * them.
     */
    ...
    /**
     * Call this method in your Activity's onSaveInstanceState to keep the BottomBar's state on
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (mBadgeMap != null && mBadgeMap.size() > 0) {
            if (mBadgeStateMap == null) {
                mBadgeStateMap = new SparseBooleanArray();
            }

            for(int i = 0, nsize = mBadgeMap.size(); i < nsize; i++) {

                BottomBarBadge badgeCandidate =
                        (BottomBarBadge) mOuterContainer.findViewWithTag(mBadgeMap.valueAt(i));

                if (badgeCandidate != null) {
                    mBadgeStateMap.put(i, badgeCandidate.isVisible());
                }
            }

            outState.putParcelable(STATE_BADGE_STATES_BUNDLE,
                    new SparseBooleanArrayParcelable(mBadgeStateMap));
        }
    ...
    /**
     * Map a background color for a Tab, that changes the whole BottomBar background color when the
     * Tab is selected.
     *
    ...
    public void mapColorForTab(int tabPosition, int color) {
        ...
        if (mIsDarkTheme || !mIsShiftingMode || mIsTabletMode) {
            return;
        }
        ...
        if (tabPosition == mCurrentTabPosition && mCurrentBackgroundColor != color) {
     ...
     * <p/>
     * NOTE: You might want to change your active tab color to something else using {@link
     * #setActiveTabColor(int)}, as the default primary color might not have enough contrast for the
     * dark background.
     */
    public void useDarkTheme() {
        if (!mIsDarkTheme && mItems != null && mItems.length > 0) {
            ...
            for (int i = 0; i < mItemContainer.getChildCount(); i++) {
                ...
                ((ImageView) bottomBarTab.findViewById(R.id.bb_bottom_bar_icon)).setColorFilter(
                        mWhiteColor);
    ...
    /**
     * Ignore the automatic Night Mode detection and use a light theme by default, even if the Night
     * Mode is on.
     */
    ...
    /**
     * Creates a new Badge (for example, an indicator for unread messages) for a Tab at the
     * specified position.
     *
     * @param tabPosition zero-based index for the tab.
    ...
    public BottomBarBadge makeBadgeForTabAt(int tabPosition, int backgroundColor,
        ...
        BottomBarBadge badge = new BottomBarBadge(mContext, tabPosition, tab, backgroundColor);
        ...
        if (mIsComingFromRestoredState && mBadgeStateMap != null && mBadgeStateMap.get(
                tabPosition, false)) {
            canShow = mBadgeStateMap.get(tabPosition);
        }

        if (canShow && mCurrentTabPosition != tabPosition && initialCount != 0) {
            badge.show();
        } else {
            badge.hide();
        }
    ...
    /**
     * Set a custom TypeFace for the tab titles. The .ttf file should be located at
     * "/src/main/assets".
     *
    ...
    /**
     * Prevent the BottomBar drawing behind the Navigation Bar and making it transparent. Must be
     * called before setting items.
     */
    ...
    /**
     * Force the BottomBar to behave exactly same on tablets and phones, instead of showing a left
     * menu on tablets.
     */
    ...
    /**
     * Don't resize the tabs when selecting a new one, so every tab is the same0 if you have more
     * than three tabs. The text still displays the scale animation and the icon moves up, but the
     * badass width animation is ignored.
     */
    ...
    /**
     * Get this BottomBar's height (or width), depending if the BottomBar is on the bottom (phones)
     * or the left (tablets) of the screen.
     *
                    ...
                    .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                        ...
                        public void onGlobalLayout() {
                            listener.onSizeReady(
                                    mIsTabletMode ? mOuterContainer.getWidth()
                                            : mOuterContainer.getHeight());
    ...
    /**
     * Get the actual BottomBar that has the tabs inside it for whatever what you may want to do
     * with it.
     *
    ...
    /**
     * If you get some unwanted extra padding in the top (such as when using CoordinatorLayout),
     * this fixes it.
     */
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mInActiveColor = ContextCompat.getColor(getContext(),
                R.color.bb_inActiveBottomBarItemColor);
    ...
    private void initializeViews() {
        mIsTabletMode = !mIgnoreTabletLayout && mContext.getResources()
                .getBoolean(R.bool.bb_bottom_bar_is_tablet_mode);
        ViewCompat.setElevation(this, MiscUtils.dpToPixel(mContext, 8));
        View rootView = View.inflate(mContext,
                mIsTabletMode ? R.layout.bb_bottom_bar_item_container_tablet
        ...
        if (mPendingUserContentView != null) {
            ...
            if (mIsTabletMode && mIsShy) {
                ((ViewGroup) mPendingUserContentView.getParent()).removeView(
                        mPendingUserContentView);
            }
                    ...
                    new ViewTreeObserver.OnGlobalLayoutListener() {
                        ...
                        public void onGlobalLayout() {
                            if (!mShyHeightAlreadyCalculated) {
                                ((CoordinatorLayout.LayoutParams) getLayoutParams()).setBehavior(
                                        new BottomNavigationBehavior(
                                                getOuterContainer().getHeight(), 0, isShy(),
                                                mIsTabletMode));
                            }
    ...
    private void updateSelectedTab(int newPosition) {
        final boolean notifyMenuListener = mMenuListener != null
        ...
        if (newPosition != mCurrentTabPosition) {
            ...
            if (notifyMenuListener) {
                notifyMenuListener(mMenuListener, false,
                        ((BottomBarTab) mItems[mCurrentTabPosition]).id);
            }
            ...
            if (notifyMenuListener) {
                notifyMenuListener(mMenuListener, true,
                        ((BottomBarTab) mItems[mCurrentTabPosition]).id);
            }
        }
    }
    ...
    private void notifyMenuListener(Object listener, boolean isReselection, @IdRes int menuItemId) {
        if (listener instanceof OnMenuTabClickListener) {
            ...
            OnMenuTabSelectedListener onMenuTabSelectedListener
                    = (OnMenuTabSelectedListener) listener;
    ...
    private void handleBadgeVisibility(int oldPosition, int newPosition) {
        ...
        if (mBadgeMap.get(oldPosition) != null) {
            BottomBarBadge oldBadge =
                    (BottomBarBadge) mOuterContainer.findViewWithTag(mBadgeMap.get(oldPosition));
        ...
        if (mBadgeMap.get(newPosition) != null) {
            BottomBarBadge newBadge =
                    (BottomBarBadge) mOuterContainer.findViewWithTag(mBadgeMap.get(newPosition));
            newBadge.hide();
        }
    }
    ...
    private boolean handleLongClick(View v) {
        if ((mIsShiftingMode || mIsTabletMode) && v.getTag().equals(TAG_BOTTOM_BAR_VIEW_INACTIVE)) {
            Toast.makeText(mContext, mItems[findItemPosition(v)].getTitle(mContext),
                    Toast.LENGTH_SHORT)
                    .show();
        }
    ...
    private void updateItems(final BottomBarItemBase[] bottomBarItems) {
        ...
        if (!mIsTabletMode) {
            int proposedItemWidth =
                    Math.min(MiscUtils.dpToPixel(mContext, mScreenWidth / bottomBarItems.length),
                            mMaxFixedItemWidth);

            mInActiveShiftingItemWidth = (int) (proposedItemWidth * 0.9);
            mActiveShiftingItemWidth =
    ...
    private void darkThemeMagic() {
        if (!mIsTabletMode) {
            ...
            mTabletRightBorder.setBackgroundColor(
                    ContextCompat.getColor(mContext, R.color.bb_tabletRightBorderDark));
        }
    }
    ...
    private void onRestoreInstanceState(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            ...
            mBadgeStateMap = savedInstanceState.getParcelable(
                    STATE_BADGE_STATES_BUNDLE);
    ...
    /**
     * Material Design specify that there should be a 10dp padding under the text, it seems that it
     * means 10dp starting from the text baseline. This method takes care of calculating the amount
     * of padding that needs to be added to the Title TextView in order to comply with the Material
    ...
    private void selectTab(View tab, boolean animate) {
        ...
        if (animate) {
            ViewPropertyAnimatorCompat titleAnimator =
                    ViewCompat.animate(title).setDuration(ANIMATION_DURATION).scaleX(1).scaleY(1);
            ...
            if (mIsShiftingMode) {
                ViewCompat.animate(icon).setDuration(ANIMATION_DURATION).alpha(1.0f).start();
            }
    ...
    private void unselectTab(View tab, boolean animate) {
        ...
        if (animate) {
            ...
            MiscUtils.resizePaddingTop(icon, icon.getPaddingTop(), iconPaddingTop,
                    ANIMATION_DURATION);

            if (mIsShiftingMode) {
                ViewCompat.animate(icon).setDuration(ANIMATION_DURATION).alpha(0.6f).start();
            }
        } else {
    ...
    private void handleBackgroundColorChange(int tabPosition, View tab) {
        ...
        if (mColorMap != null && mColorMap.get(tabPosition, -1) != -1) {
    ...
    private void handleBackgroundColorChange(View tab, int color) {
        MiscUtils.animateBGColorChange(tab, mBackgroundView, mBackgroundOverlay, color);
        mCurrentBackgroundColor = color;
    }
    ...
    private void updateCurrentFragment() {
        if (!mShouldUpdateFragmentInitially
    ...
    private static void navBarMagic(Activity activity, final BottomBar bottomBar) {
        ...
        if (!bottomBar.drawBehindNavBar() || navBarHeight == 0 || (!(softMenuIdentifier > 0
                && res.getBoolean(softMenuIdentifier)))) {
            return;
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH
                && ViewConfiguration.get(
                activity).hasPermanentMenuKey()) {
            return;
        }
            ...
            bottomBar.getViewTreeObserver()
                    .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                        ...
                        public void onGlobalLayout() {
                            ...
                            if (bottomBar.isShy()) {
                                int defaultOffset = bottomBar.useExtraOffset() ? navBarHeightCopy
                                        : 0;
                                bottomBar.setTranslationY(defaultOffset);
                                ((CoordinatorLayout.LayoutParams) bottomBar.getLayoutParams()).setBehavior(
                                        new BottomNavigationBehavior(newHeight, defaultOffset,
                                                bottomBar.isShy(),
                                                bottomBar.mIsTabletMode));
                            }
